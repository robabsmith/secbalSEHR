...
address:
- address: MaMTEP, Department of Business and Management, Fibigerstr{\ae}de 2, Aalborg East
  code: 9220
- address: MaMTEP, Department of Business and Management, Fibigerstr{\ae}de 2, Aalborg East
  code: 9220
...
DK_rolling_result_P_G <- roll_cor(DK_rolling_P_G, years_rolling)
DK_rolling_result_P_R <- roll_cor(DK_rolling_P_R, years_rolling)
DK_rolling_result_G_R <- roll_cor(DK_rolling_G_R, years_rolling)
...
NO_rolling_result_P_G <- roll_cor(NO_rolling_P_G, years_rolling)
NO_rolling_result_P_R <- roll_cor(NO_rolling_P_R, years_rolling)
NO_rolling_result_G_R <- roll_cor(NO_rolling_G_R, years_rolling)
...
SE_rolling_result_P_G <- roll_cor(SE_rolling_P_G, (years_rolling))
SE_rolling_result_P_R <- roll_cor(SE_rolling_P_R, (years_rolling))
SE_rolling_result_G_R <- roll_cor(SE_rolling_G_R, (years_rolling))
...
[as some examples, @trehan1991; @blanchard2002; @bernanke2005; @blanchard2007; @vansteenkiste2008]
@aliabbas2011
@christensenetal1995
...
Krugman (2009) uses the sectoral balances to explain what happened in
...
the System is more or less cut in stone. (Bergman 2015)

Background
----------

In Christensen et. al. (1995) a survey on the Nordic National Account
...
do mention however, that already in the 1930s Frisch emphasizes the
importance in the distinction between real(goods and services) and
financial objects(claims and counterclaims), Christensen et. al. (1995,
35). Despite this point from Frisch, the financial side of the accounts
didn't play a major role in the national account. The lack of
...
including revaluation account, were given. (Abildgaard 2008)

In 1990 the Norwegian national account system was updated with the
purpose of fulfilling the recommendations in the SNA 1993 and the
European standard in ESA 1995. Regarding the sectoral balances two
...
significantly(Statistics Norway 2012). According to Statistic Norway
2012, during the period of 1950s to 1990, the financial part of the
...
Market Statistics: Financial Sector Balance Sheet'(see e.g. Statistics
Norway 1980) the stock of a number of financial assets can be seen at a
...
to follow the SNA 1993 and ESA 1995. According to Abildgaard (2008),
Statistic Denmark didn't integrate financial account stock- and
flow-data in the annual national accounts statistics before 2001. The
financial sectoral data provided since 2001 covers the period since
primo 1995. Regarding the non-financial sectoral national account, no
...
financial side of the economy[^4].

In Bergman et. al (2010) the sectoral balances for the Swedish economy
...
sectoral net lending back to 1810, Waldenström (2017). From 1880 to 1901
both Bollfras and Fahlbeck (Waldenström -- appendix) gave some estimates
...
created a financial balance sheet for all the sectors from 1945-1990.
...
bracket represents the sector balance for *RoW*[^5]. Broadly speaking,
...
capital gains(or revaluations). As pointed out by Waldenström (2016) and
Knudsen (2017) fluctuations in net wealth over time are mainly due to
...
the system of national accounts (SNA). The origin of standardized system
...
System of National Accounts. xxx trace the development of the SNA back
to an article published by Meade and Stone at the Great Britain Treasury
-- although the inspiration can also attributed further back to Keynes
and many others such as François Quesnay. As noted by xxx, the initial
interest of Copeland in the 1940s was to investigate the financing
requirements of economic activity between and across the national
sectors[^6]. The SFB approach could be described as a continuation of
Copeland's goals with the aid of modern technology. The work of Wynne
Godley together with other key authors is recognized by many economist
as the instigating factor in the re-emergence of *SFB*. In light of the
...
The REH theory, introduced by xxx and more recently reviewed by xxx,
explains that as *Govt* borrows, households (*Hh*), and *Pvt* in
general, expect that *Govt* will need to repay its debt in the future.
The key assumption in this case it is that *Pvt* is able to anticipate
and interpret the impact of the net borrowing of *Govt* on their current
and future consumption possibilities, and respond in a rational manner
by 'smoothing' inter-temporal consumption patterns. The response is
therefore to save in times when *Govt* increases spends.

The Twin Deficits Hypothesis has been investigated extensively xxx, and
in particular with respect to the US economy . According to this
...
recent paper, Glötz xxx, present an analysis that follows the work
presented in xxx, where the test the REH and TDH against some empirical
evidence. In xxx the analysis is carried out for 22 European countries
...
neither of which show significant patterns for Sweden. In , the REH and
TDH are rejected, fully in line with the conclusion in xxx.

In xxx a graphical presentation of *NL* of the five institutional
...
xxx categorized the two theories as emerging from *Intra-temporal
trade*, and *Inter-temporal responses* effects. The former describes the
impact of fiscal policy on domestic demand, and thus relative purchasing
power (or the real exchange rate) (TDH), and the latter describes
Ricardian type inter-temporal adjustment by *Pvt* (REH). Their
presentation allows for a more nuanced understanding of the
...
departure for their analysis is that They noted that such studies can be
divided into three groups, the first, causality tests and VARs. Second,
, and the third as a combination of narrative and regression analysis to
examine the effects on the foreign balance. They drew a similar
conclusion from a mixed method analysis, including a series of panel
regressions, an analysis of large changes in fiscal and current account
balances, and a quarterly structural VAR.
...
specific event using the sectoral balances approach. In Van Treeck et.
al., an empirical analysis of the three sector balance is used to
discuss the possible outcomes of the Stability Pact program from
2011-2014, where the authors seriously question the reliability of
predicted results from the European Commission. Given the predictions of
the growth rates in each economy and the assumption for the financial
balance for *Govt* and *RoW*, they simulate and evaluate the implicitly
assumed prediction for *Pvt*. In Brink(1983) he argues that the major
reason for the negative *Govt* *NL* in Denmark during the 1970s and
...
savings exceeded the investments. As stated by Krugman (2009), the
government deficit, is due to automatic stabilizers rather than
expansionary fiscal policy. It doesn't imply however, that discretionary
policy doesn't affect the balance, since these policies directly has an
effect on the balances in the private sector as well as the Rest of the
World. In case of a lower propensity to import in the *Govt* compared to
the private sector, an increase in the public consumption financed by an
increase in taxes leads to an increase in *NL* in *Pvt*, while the *NL*
in RoW falls(improvements of the current account). (Brink 1983).

BIS (2016) uses the banking crisis in Sweden in the early 1990s as a
...
following the crisis (in figure 2). As a result of this deleveraging in
...
balance again. In a working paper from Statistics Sweden, Bergman et.
al.(2010) analyses the changes in balance sheets across the different
...
Figure 1 shows *NL* as well as NFW for the three main sectors for
Denmark.
...
```
Figure 1: Net lending and financial net wealth, Denmark.
...
DØRS?, DKK 1 of pension contributions increases the overall savings for
...
which deteriorates the current account (XXX?). In order to boost Danish
...
Figure \ref{fig:B9BF90DK}b shows NFW of the
...
cumulative result in Figure \ref{fig:B9BF90DK}a and
*NL* in Figure \ref{fig:B9BF90DK}b. The effect of
...
economy, as discussed by (BRINK?).

According to (XXX?), in the 1950s and 1960s Denmark and Sweden They
...
In Denmark the net financial wealth (NFW) of *Govt* was -40% of GDP in
...
2007 and 2009[^7]. The second appears as a rapid accumulation following
2009, where the sector as a whole increased its financial NFW to $60\%$
of GDP --- a dramatic consolidation of the sector[^8]

Figure 2 shows *NL* as well as NFW for the three main sectors for
...
```
Figure 2 Net lending and financial net wealth, Sweden
...
firms[^9]. This jeopardised export based economic growth policies and
...
1995 to 2016[^10]. The trend in the net asset position of *RoW*,
...
Interestingly, xxxx noted that using a flat rate tax deduction (though
omitting progressive tax rates) on interest payments, the Swedish
household sector had negative costs of borrowing from before 1960 until
1990. and both noted extensive deregulation of both the Danish and
Swedish financial markets between 1980 and 1990, the last of which for
Sweden included deregulation of international capital flows and the
establishment of a new money market with options and derivatives. These
deregulations permitted Swedish households to take advantage of the low
interest rate environment. explained that 1992 saw massive credit losses
in the Swedish *Pvt*, associated with the deregulations that preceded.
This was combined with an international economic slowdown, a European
currency crisis, and a *Govt* financial crisis -- followed by a
reconstruction of the tax system. All of which led to the abandonment of
the official fixed exchange rate system, a massive depreciation of the
Swedish Krona and the start of the worst crisis in Sweden since the
1930s. Unemployment rose from 1.5% to 8.2%, together with a substantial
decline in per capita GDP (adjusted for international purchasing power).

In Figure 3, *NL* as well as NFW for the three main sectors for Norway
...
Figure 3 Net Lending and Financial net Wealth, Norway
...
Pension Fund Global[^11]. Up through the 1970s and 1980s the current
...
in 2013 . In the latter half of the 1980s the combination of low
...
founded, and as noted by , changed the state of the Norwegian economy.
...
As explained, by investing the surplus from the oil production abroad,
the rate of return was independent of the Norwegian economy and the
interest and property income of the fund could be used to stabilize the
economy in times with low economic activity. The decision to invest the
surplus, rather than stimulating the economy immediately, was also to
avoid overheating the economy and to preserve the oil revenues for
future generations. Oil revenues take several forms for the Norwegian
*Govt*, including oil taxes, licenses for exploration, dividends from
the co-ownership of Statoil and interest from foreign investments.
Another important change in the structure of the Norwegian economy was
the choice of exchange rate regime. According to , in 1978 Norway
decided to leave the *currency snake* and link the Norwegian Krone to a
trade-weighted basket of currencies . In the first half of the 1980s,
much the same as for Denmark and Sweden, several devaluations were
enacted due to reduced competitiveness of Norwegian firms. In 1986 the
Krone was devalued again as a result of sharp reversal from a current
account surplus to a deficit. This was caused in part by lower oil
prices and partly due to a surge in domestic demand together with
diminished cost competitiveness, which fueled import demand. For a brief
period, from 1990 to 1992, the Krone was pegged to the ECU, before
switching to a managed floating regime.
...
*RoW*, and marginal changes for *Pvt*. This is confirmed by Figure 3,
...
The output gap[^12] data was available from the OECD database from 1985
...
between potential GDP and actual GDP.[^13] For robustness check, we used
...
Following the method presented in , a combination of national accounting
...
WHAT ABOUT A FIVE SECTOR?

### The composition of net lending

For each sector the variance in *NL* can be decomposed into its
covariance[^14] with *NL* of the other sectors, which can be seen in
Figure [\[fig:cov\_bars\_3sector\_Long\]](#fig:cov_bars_3sector_Long)
below.[^15] In order to simplify the discussion, we will simply refer to
...
Figure 1: Covariances between net lending
...
minor (and insignificant) negative changes in *Govt*[^16]. *Pvt* has a
...
*RoW* as stated above and *Govt*[^17]. A positive change in *Pvt* is
...
### Business cycle covariance of sectors

This analysis illustrates the covariance between the sector in question
and the level of the output gap (*OG*) on an annual basis, for three
lagged and three leading years. This analysis allows us to find the sign
of the covariance between *NL* in a single sector and the *OG*, in order
to say something about the behavior over the business cycles. It also
lets us see if a positive or negative net position for the sector in
question leads or lags the business cycle, where position $0$ indicates
a concurrent movement between sector net position and the *OG*.

The results of this three-sector business cycle covariance test are
illustrated in Figure XX
below.[^18]

```{r buscycDK3sectlong, fig.cap="DK: Covariance", out.width="0.2\\linewidth", fig.height=5, fig.width=5, echo=FALSE, warning=FALSE, message=FALSE}

plot(DK_CCF_Long$L,DK_CCF_Long$pvt,type="l",col=c("#ffc100"), lty=c(1),ylim=c(-1,1),ylab="Covariance",xlab="Lags")
lines(DK_CCF_Long$L,DK_CCF_Long$g,type="l",col=c("#443eff"), lty=c(2))
lines(DK_CCF_Long$L,DK_CCF_Long$row,type="l",col=c("#107831"), lty=c(3))
lines(DK_CCF_Long$L,DK_CCF_Long$pvt_sig_points,type="p",col=c("#443eff"), pch=c(1))
lines(DK_CCF_Long$L,DK_CCF_Long$g_sig_points,type="p",col=c("#443eff"), pch=c(2))
lines(DK_CCF_Long$L,DK_CCF_Long$row_sig_points,type="p",col=c("#107831"), pch=c(3))
grid()
abline(h=0, col="#000000")
legend("bottomright",col=c("#ffc100", "#443eff", "#107831"), lwd=1.75,lty=c(1:3), legend=c("Priv","Govt","RoW"),bty='n')
``` 

```{r buscycNO3sectlong, fig.cap="NO: Covariance", out.width="0.2\\linewidth", fig.height=5, fig.width=5, echo=FALSE, warning=FALSE, message=FALSE}

plot(NO_CCF_Long$L,NO_CCF_Long$pvt,type="l",col=c("#ffc100"), lty=c(1),ylim=c(-1,1),ylab="Covariance",xlab="Lags")
lines(NO_CCF_Long$L,NO_CCF_Long$g,type="l",col=c("#443eff"), lty=c(2))
lines(NO_CCF_Long$L,NO_CCF_Long$row,type="l",col=c("#107831"), lty=c(3))
lines(NO_CCF_Long$L,NO_CCF_Long$pvt_sig_points,type="p",col=c("#443eff"), pch=c(1))
lines(NO_CCF_Long$L,NO_CCF_Long$g_sig_points,type="p",col=c("#443eff"), pch=c(2))
lines(NO_CCF_Long$L,NO_CCF_Long$row_sig_points,type="p",col=c("#107831"), pch=c(3))
grid()
abline(h=0, col="#000000")
legend("bottomright",col=c("#ffc100", "#443eff", "#107831"), lwd=1.75,lty=c(1:3), legend=c("Priv","Govt","RoW"),bty='n')
``` 

```{r buscycSE3sectlong, fig.cap="SE: Covariance", out.width="0.2\\linewidth", fig.height=5, fig.width=5, echo=FALSE, warning=FALSE, message=FALSE}

plot(SE_CCF_Long$L,SE_CCF_Long$pvt,type="l",col=c("#ffc100"), lty=c(1),ylim=c(-1,1),ylab="Covariance",xlab="Lags")
lines(SE_CCF_Long$L,SE_CCF_Long$g,type="l",col=c("#443eff"), lty=c(2))
lines(SE_CCF_Long$L,SE_CCF_Long$row,type="l",col=c("#107831"), lty=c(3))
lines(SE_CCF_Long$L,SE_CCF_Long$pvt_sig_points,type="p",col=c("#443eff"), pch=c(1))
lines(SE_CCF_Long$L,SE_CCF_Long$g_sig_points,type="p",col=c("#443eff"), pch=c(2))
lines(SE_CCF_Long$L,SE_CCF_Long$row_sig_points,type="p",col=c("#107831"), pch=c(3))
grid()
abline(h=0, col="#000000")
legend("bottomright",col=c("#ffc100", "#443eff", "#107831"), lwd=1.75,lty=c(1:3), legend=c("Priv","Govt","RoW"),bty='n')
``` 

These results provide some useful information, despite the fact that the
test for lags or leads do not show any clear patterns[^19]. All three
countries have negative covariance between *Pvt* and *OG*. This
indicates that an a positive *OG* is associated with negative *NL*,
which can be interpret as pro-cyclical *NL* behaviour (the sector thus
net-borrows as capacity utilization rises). *NL* of *Govt* is positively
related with the *OG*, so that an increase in the *OG* is associated
with positive *NL*. This can be interpret as counter-cyclical *NL*
behaviour (or stabilization borrowing and spending by *Govt* during
downturns, and vice versa for expansionary periods), as it dampens the
business cycle. For *RoW* no clear story can be told, since the
correlation values are mostly insignificant and the sign depends
strongly on the chosen number of lags.

The relevance of this analysis is to shed light on the likely behaviours
of the sectors at different points of the business cycle. For example,
if *Govt* is likely to spend (and borrow) counter-cyclically, and *Pvt*
pro-cyclically, then a negative correlation between *Pvt* and *Govt* may
exist in the absence of any rational expectations. The obscurity of the
relationship between *RoW* and *OG* may also cause us to question the
notion that expansions in aggregate demand necessarily lead to an
increase in imports, as the TDH would have us believe. Equally, the
counter-cyclical spending of *Govt* may have nothing to do with
intentions, and could simply be the result of pro-cyclical changes in
the level of tax income in conjunction with sticky expenditures. To help
grasp some of these potential dependencies we construct a simple model
that incorporates the three sectors, while satisfying the *NL* identity
in Equation $ID.2$.
...
Denmark or Sweden, then the NL of the *Govt* automatic decreases? A good
starting point for that discussion is to use an example from Sweden; the
banking crisis in the early 1990s. As a result of the crisis, the
private sector wanted to deleverage as recognized by BIS (2016), which
can be seen by the positive net lending in the private sector in
Sweden(in figure 2). As a result of this deleveraging in the private
...
and actually ended up being worse, than in the case of Sweden. (BIS
2016)
